...
from dreamer import PointsType
...
def scatter_plot(points: PointsType):
...
def simulate(
    agent,
    envs,
    cache: Dict[int, Dict],  # eps for envs, id: env.id, value = {'state', 'action', etc.}
    directory,
    ...
    episodes=0,
    state=None,
    points=None,  # collect of all points
):
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if done.any():  # (envs,)
            ...
            for index, result in zip(indices, results):
                ...
                add_to_cache(cache, envs[index].id, t, points)
        ...
        for a, result, env in zip(action, results, envs):
            ...
            add_to_cache(cache, env.id, transition, points)
...
def add_point(trans: dict, points: PointsType):
    points.append(tuple(trans["state"][:2]))


def add_to_cache(cache, env_id, transition, points=None):
    if points is not None:
        add_point(transition, points)
...
def sample_episodes(episodes, length, seed=0):
    ...
    while True:
        ...
        p = p / np.sum(p)
        while size < length:
            episode = np_random.choice(
                list(episodes.values()), p=p
            )  # chose from episodes envs
